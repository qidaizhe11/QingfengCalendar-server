...
// conn_handle 从0开始递增，以防止因socket handle重用引起的一些冲突
...
void httpconn_callback(void* callback_data, uint8_t msg, uint32_t handle, uint32_t uParam,
                       void* pParam)
{
    NOTUSED_ARG(uParam);
    NOTUSED_ARG(pParam);
...
void CHttpConn::_HandleMsgServRequest(string &url, string &post_data)
{
    if (g_msg_serv_info.size() < 0) {
        ...
        snprintf(szContent, HTTP_RESPONSE_HTML_MAX, HTTP_RESPONSE_HTML, strContent.length(),
                 strContent.c_str());
