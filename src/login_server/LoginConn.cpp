...
void login_conn_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    ConnMap_t::iterator it =  g_client_conn_map.begin();
    for (; it != g_client_conn_map.end(); ) {
    ...
    it = g_msg_serv_conn_map.begin();
    for (; it != g_msg_serv_conn_map.end(); ) {
        ConnMap_t::iterator it_old = it;
        ++it;
    ...
    if (m_handle != NETLIB_INVALID_HANDLE) {
        ...
        if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
            ...
            if (it != g_msg_serv_info.end()) {
                ...
                log("onclose from msgserver: %s:%s", pMsgServInfo->hostname.c_str(), pMsgServInfo->port);
                delete pMsgServInfo;
...
void CLoginConn::_HandleMsgServInfo(CImPdu *pPdu)
{
    ...
    log("MsgServInfo, ip_addr1=%s, ip_addr2=%s, port=%d, max_conn_cnt=%d, cur_conn_cnt=%d, "\
        "hostname: %s.",
        pMsgServInfo->ip_addr1.c_str(), pMsgServInfo->ip_addr2.c_str(), pMsgServInfo->port,
        pMsgServInfo->max_conn_cnt, pMsgServInfo->cur_conn_cnt, pMsgServInfo->hostname.c_str());
}
...
void CLoginConn::_HandleMsgServRequest(CImPdu *pPdu)
{
    ...
    map<uint32_t, msg_serv_info_t*>::iterator it = g_msg_serv_info.begin();
    for (; it != g_msg_serv_info.end(); ++it) {
